...
```{r}
C <- matrix(c(1, 1, 0, 0, 1, 1, 0, 0, 1), nrow = 3, byrow = TRUE)
C_inv <- solve(C)
```
...
```{r}
apply(C_inv, MARGIN = 1, FUN = median)
C_inv
```
...
Flag to determine whether missing vals should be removed or not

- Look at the documentation for `rowSums()`. What subtle difference is there between `rowSums(A)` and `apply(X = A, MARGIN = 1, FUN = sum)`?

*Answer*
- Faster than apply
If na.rm = FALSE and either NaN or NA appears in a sum, the result will be one of NaN or NA, but which might be platform-dependent.
...
```{r}
# your code goes here
grades[grades$score>87, c("score", "student_id")]
```
...
```{r}
saveRDS(zv, "zv_data.rds")
rm(zv)
zv
zv <- readRDS("zv_data.rds")
zv
```
...
```{r}
x <- seq(from = -3,to = 3, by = 0.01)
density <- dunif(x = x, min = -3, max = 3, log = FALSE)
...
```{r}
for (i in 1:5) {
  print(paste(i, " is ", if(i %% 2 == 1) "odd" else "even"))
}
```
...
```{r}
genRnd <- function() {
  return(round(runif(1, min=5, max=10)))
}
```
...
printVals <- function() {
  ...
  for (i in 1:10) {
    val <- genRnd()
    print(paste(val, " is ", if (val %% 2 == 0) "even" else "odd"))
  }
}
